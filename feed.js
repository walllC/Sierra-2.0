document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderRight() {
    ...
    if (sl) {
      ...
      Storage.getSuggestedUsersAsync().then(suggested => {
        ...
        // Replaced navigate_msg() with inline navigate call
    ...
    Storage.getUserByUsernameAsync(ME.username).then(user => {
      ...
      modal.querySelector('#ep-save').addEventListener('click', async () => {
        ...
        if (avFile) {
          ...
          if (!res.success) { Utils.showToast(res.error || 'Avatar upload failed', 'error'); saveBtn.disabled = false; saveBtn.textContent = 'Save'; return; }
        }
        ...
        if (covFile) {
          ...
          const res = await fetch('upload_image.php', { method: 'POST', body: fd }).then(r => r.json());
          if (!res.success) { Utils.showToast(res.error || 'Cover upload failed', 'error'); saveBtn.disabled = false; saveBtn.textContent = 'Save'; return; }
        }
  ...
  function loadComments(rantId, container) {
    ...
    fetch(`get_comments.php?rant_id=${rantId}`)
      ...
      .catch(() => { list.innerHTML = `<div style="font-size:13px;color:var(--danger);padding:8px 0">Failed to load comments.</div>`; });
  }
  ...
  // ════════════════════════════════════════
  // RANT MODAL (sidebar "+ Post Rant" button)
  // ════════════════════════════════════════

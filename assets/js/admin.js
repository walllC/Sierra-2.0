document.addEventListener('DOMContentLoaded', async () => {
    ...
    navBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (btn.dataset.sec === 'comments')  renderComments();
    ...
    async function renderUsers() {
        ...
        users.forEach(user => {
            ...
            tr.querySelector('.ban-btn').addEventListener('click', async () => {
                ...
                if (newStatus === 'banned') {
                    await apiRequest('api/admin_delete_user_data.php', { id: user.id, username: user.username });
                }
    ...
    async function renderComments() {
        ...
        const comments = await apiRequest('api/admin_get_comments.php');

        if (!comments.length) {
            list.innerHTML = `<div class="empty"><p>No comments yet.</p></div>`;
            return;
        }
        ...
        comments.forEach(c => {
            ...
            const isReply = c.parent_id && parseInt(c.parent_id) > 0;

            card.innerHTML = `
                <div class="a-rant-body">
                    <div class="a-rant-meta">
                        <span class="name">@${Utils.escapeHtml(c.username)}</span>
                        ${isReply ? `<span class="badge" data-val="reply" style="margin-left:6px">Reply</span>` : ''}
                        <span class="time">· ${formatDate(c.created_at)}</span>
                    </div>
                    <div class="a-rant-text">${Utils.escapeHtml(c.content || '')}</div>
                    <div class="a-comment-rant-ref">
                        🗒️ On rant: <em>${Utils.escapeHtml(c.rant_content || '')}</em>
                    </div>
                </div>
                <button class="btn btn-danger-soft btn-xs del-comment-btn" data-id="${c.id}">Delete</button>`;

            card.querySelector('.del-comment-btn').addEventListener('click', async (e) => {
                if (!confirm('Delete this comment?')) return;
                await apiRequest('api/admin_delete_comment.php', { id: e.target.dataset.id });
                renderComments();
                renderOverview();
            });
